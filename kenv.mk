...
###############################
define cp-hct-share-to-kernel-config
touch $(HCT_SHARE_CONFIG_FILE);\
echo some hct_share_config_begin $(1) $(2);\
startLine=`grep -n "hct_share_config_begin" $(2) | head -1 | cut -d ":" -f 1`;\
if [ $$startLine -ne 0 ];then \
echo some hct_share_config_begin find,$$startLine;\
endLine=`cat $(2) | wc -l`;\
sed -i "$$startLine"','$$endLine'd' $(2);\
fi;\
startLine2=`grep -n "hct_custom_config_begin" $(2) | head -1 | cut -d ":" -f 1`;\
if [ $$startLine2 -ne 0 ];then \
echo some hct_custom_config_begin find,$$startLine2;\
endLine2=`cat $(2) | wc -l`;\
sed -i "$$startLine2"','$$endLine2'd' $(2);\
fi;\
cat $(HCT_SHARE_CONFIG_FILE)>>$(KERNEL_CONFIG_FILE)
endef
#########################
###############################
define cp-hct-config-to-kernel-config
echo some hct_custom_config_begin $(1) $(2);\
...
ifneq ($(strip $(TARGET_NO_KERNEL)),true)
  ...
  HCT_SHARE_CONFIG_FILE:=$(KERNEL_DIR)/arch/$(TARGET_ARCH)/configs/hct_share_config
