...
#ifdef CONFIG_TCPC_MT6360
extern unsigned int battery_get_input_current(void);
#endif
...
#ifdef CONFIG_TCPC_MT6360
	bat_data->BAT_BatteryAverageCurrent = battery_get_input_current();
	//printk("hct_jk [%s] bat_data->BAT_BatteryAverageCurrent= %d ! \n" ,__func__,bat_data->BAT_BatteryAverageCurrent);
#else
	bat_data->BAT_BatteryAverageCurrent = ((battery_get_bat_current()) / 10) > 0 ? ((battery_get_bat_current()) / 10) : (-((battery_get_bat_current()) / 10));
#endif
}
