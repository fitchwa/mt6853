...
static int hct_customkey_f3_irqnum;

static unsigned int hct_customkey_f3_gpio_num;
...
static void kpd_customkey_f3_handler(unsigned long data)
{
	...
	customkey_f3_state = gpio_get_value(hct_customkey_f3_gpio_num);//__gpio_get_value(hct_customkey_f3_gpio_num);
	if(customkey_f3_state < 0)
		return;
	printk("%s: f3_num = (%d) customkey_f3_state = (%d) gpio_get_value end. \n", __func__,hct_customkey_f3_gpio_num,customkey_f3_state);
	...
	if (pressed)
	{
		...
		irq_set_irq_type(hct_customkey_f3_irqnum, IRQF_TRIGGER_RISING);
	}
	else
	{
		printk("%s: key %s. \n", __func__, pressed ? "pressed" : "released");
		irq_set_irq_type(hct_customkey_f3_irqnum, IRQF_TRIGGER_FALLING);
	}

	enable_irq(hct_customkey_f3_irqnum);
}

static irqreturn_t hct_customkey_f3_isr(int irqnum, void *data)
{
	printk("%s: enter.\n", __func__);

	disable_irq_nosync(hct_customkey_f3_irqnum);
 ...
 */
int hct_customkey_f3_init(struct platform_device *pdev)
{
	int retval;
	//int err = -EIO;
	...
	/*get the node*/
	node = of_find_compatible_node(NULL, NULL, "mediatek,hct_customkey3");
	if (node) {
        printk("%s: node = (%d) not null.\n", __func__,node);

		hct_customkey_f3_irqnum = irq_of_parse_and_map(node, 0);
		printk("%s: f3  hct_customkey_f3_irqnum = %d\n", __func__, hct_customkey_f3_irqnum);
	}
	else {
		printk("%s: f3  cannot get the node: 'mediatek,hct_customkey'.\n", __func__);
		return -ENODEV;
	}
	...
	hct_customkey_f3_gpio_num = hct_parse_dts(node, CUSTOMKEY_F3_GPIO_NAME, false);


	if(hct_customkey_f3_gpio_num < 0)
	{   //获取GPIO num 失败
	...
	{	//打印出GPIO num的值
		printk("%s = %d. \n",CUSTOMKEY_F3_GPIO_NAME, hct_customkey_f3_gpio_num);
	}

	/*irq register*/
	retval = request_irq(hct_customkey_f3_irqnum, hct_customkey_f3_isr, IRQF_TRIGGER_FALLING, "customkeyf3", NULL);
	if (retval != 0) {
		printk("%s: request_irq fail, ret %d, irqnum %d.\n", __func__, retval, hct_customkey_f3_irqnum);
		return retval;
	}
	enable_irq(hct_customkey_f3_irqnum);

	printk("+++++++++++ hct_customkey_f3_init end +++++++++++");
	return retval;
}
