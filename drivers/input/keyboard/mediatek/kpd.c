...
/* hct-drv add for customkey F1 by jk begin */
...
extern int hct_customkey_f2_init(struct platform_device *pdev);
	#endif
#endif
#ifdef __HCT_CUSTOMKEY_F3_SUPPORT__
	#if __HCT_CUSTOMKEY_F3_SUPPORT__
extern int hct_customkey_f3_init(struct platform_device *pdev);
	#endif
#endif
/* hct-drv add for customkey F1 by jk end */
...
void mobile_source_handler(bool open)
{
	...
	if(open)
	{
		if(!kpd_input_dev)
		{
			...
			printk("mobile_source_handler1111 = %d\n", MOBILESOURCE_OPEN_KEY);
	}
	else
	{
		if(!kpd_input_dev)
...
#ifdef __HCT_CUSTOMKEY_F2_SUPPORT__
	#if __HCT_CUSTOMKEY_F2_SUPPORT__
	kpd_print("__set_bit F2 begin.\n");
		hct_customkey_f2_init(pdev);
		__set_bit(KEY_F2, kpd_input_dev->keybit);
	kpd_print("__set_bit F2 end.\n");
	#endif
#endif
#ifdef __HCT_CUSTOMKEY_F3_SUPPORT__
	#if __HCT_CUSTOMKEY_F3_SUPPORT__
	kpd_print("__set_bit F3 begin.\n");
		hct_customkey_f3_init(pdev);
		__set_bit(KEY_F3, kpd_input_dev->keybit);
	kpd_print("__set_bit F3 end.\n");
	#endif
#endif	
/* hct-drv add for customkey F1 by jk end */
