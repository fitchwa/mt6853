...
static irqreturn_t tpd_eint_handler(int irq, void *data);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	dev_err(rmi4_data->pdev->dev.parent,
			"%s: hct-drv. Function %02x reporting\n",
		...
		bool report)
{
	...
	hct_synaptics_debug("%s  line: %d.	status.data[0] = %d. \n", __func__, __LINE__, status.data[0]);		
...
#if 0 //hct-drv disabled it, we use work.
...
static int hct_count = 0;
static irqreturn_t tpd_eint_handler(int irq, void *data)
{
	struct synaptics_rmi4_data *rmi4_data = data;

	hct_synaptics_debug("%s  line: %d.	hct_count = %d. \n", __func__, __LINE__, ++hct_count);		
...
static int touch_event_handler(void *data)
{
	...
	do{
		...
		*/
		...
		hct_synaptics_debug("%s  line: %d.	hct_count2 = %d. \n", __func__, __LINE__, ++hct_count2);		
		...
		if (intr_mask[ii] != 0x00) {
			...
			if (enable) {
				retval = synaptics_rmi4_reg_write(rmi4_data,
						...
						&(intr_mask[ii]),
						sizeof(intr_mask[ii]));
				hct_synaptics_debug("%s  line: %d. enable. retval = %d.  \n", __func__, __LINE__, retval);
				...
				retval = synaptics_rmi4_reg_write(rmi4_data,
						...
						&zero,
						sizeof(zero));
				hct_synaptics_debug("%s  line: %d. disable. retval = %d.  \n", __func__, __LINE__, retval);
				if (retval < 0)
					return retval;
			}
		}
		...
		bool enable, bool attn_only)
{
	...
	if (enable) {
		...
		enable_irq(rmi4_data->irq); //hct-drv add.

		retval = synaptics_rmi4_int_enable(rmi4_data, true);
		hct_synaptics_debug("%s  line: %d. retval = %d.  \n", __func__, __LINE__, retval);
		if (retval < 0)
			goto exit;
		...
		if (rmi4_data->irq_enabled) {
			...
			//free_irq(rmi4_data->irq, rmi4_data); //hct-drv disabled.
		...
		bool get)
{
	...
	const struct synaptics_dsx_board_data *bdata =
			rmi4_data->hw_if->board_data;
	hct_synaptics_debug("%s  line: %d. get = %d. \n", __func__, __LINE__, get);

	if (!get) {
		retval = 0;
		goto regulator_put;
	}
	hct_synaptics_debug("%s  line: %d. bdata->pwr_reg_name = %s. \n", __func__, __LINE__, bdata->pwr_reg_name);

	if ((bdata->pwr_reg_name != NULL) && (*bdata->pwr_reg_name != 0)) {
		rmi4_data->pwr_reg = regulator_get(rmi4_data->pdev->dev.parent,"vtouch");
		...
		bool enable)
{
	...
	if (rmi4_data->pwr_reg) {
		retval = regulator_set_voltage(rmi4_data->pwr_reg,3000000,3000000);
		hct_synaptics_debug("%s  line: %d. retval = %d. \n", __func__, __LINE__, retval);
	}
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	hct_synaptics_debug("%s  line: %d.  rmi4_data->irq = %d. \n", __func__, __LINE__, rmi4_data->irq);
	...
	//retval = request_threaded_irq(rmi4_data->irq, NULL,synaptics_rmi4_irq, bdata->irq_flags,PLATFORM_DRIVER_NAME, rmi4_data);
	//retval = request_irq(rmi4_data->irq, synaptics_rmi4_irq, IRQ_TYPE_EDGE_FALLING, PLATFORM_DRIVER_NAME, rmi4_data);
	retval = request_irq(rmi4_data->irq, tpd_eint_handler, IRQ_TYPE_EDGE_FALLING, PLATFORM_DRIVER_NAME, rmi4_data);
	
	
	hct_synaptics_debug("%s  line: %d. retval = %d. rmi4_data->irq = %d. thread = %p. \n", __func__, __LINE__, retval, rmi4_data->irq, thread);
	if (retval < 0) {
		dev_err(&pdev->dev,
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	hct_synaptics_debug("%s  line: %d.	rmi4_data->stay_awake = %d. \n", __func__, __LINE__, rmi4_data->stay_awake);
	dump_stack();
