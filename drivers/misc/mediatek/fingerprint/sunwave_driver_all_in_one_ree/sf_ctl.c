...
static int sf_read_sensor_id(void)
{
    ...
    char writebuf[16] = {0};
    int cfg_len = 0;
#if (!SF_TRUSTKERNEL_COMPATIBLE) || defined(__HCT_FINGER_SUN_GOODIX_COMPATIBLE_SUPPORT__)
	int retry_8201 = 0;//micheal
#endif
    ...
    do {
		...
		//xprintk(KERN_INFO, " jk add tee_spi_transfer.readbuf = 0x%02x-%02x-%02x-%02x-%02x-%02x-%02x \n", readbuf[0], readbuf[1], readbuf[2], readbuf[3], readbuf[4], readbuf[5], readbuf[6]);
