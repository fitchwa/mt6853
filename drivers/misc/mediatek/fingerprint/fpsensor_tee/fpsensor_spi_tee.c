...
#if 0//FPSENSOR_TKCORE_COMPATIBLE
...
#ifdef __HCT_FINGER_SUN_CHIPONE_COMPATIBLE_SUPPORT__
...
static int fpsensor_irq_gpio_cfg(fpsensor_data_t *fpsensor)
{
    ...
    if (node) {
        ...
        fpsensor_debug(ERR_LOG,"[fpsensor]ints[0] = %d,is irq_gpio , ints[1] = %d!!\n", ints[0], ints[1]);
...
#else
    fpsensor->vdd_regu = regulator_get(&fpsensor->spi->dev, "irtx_ldo");
    printk("hctdrv add %s is regulator_get !!!\n",__func__);
    if (IS_ERR(fpsensor->vdd_regu)) {
        ret = PTR_ERR(fpsensor->vdd_regu);
        printk("Regulator get failed vdd err = %d\n", ret);
        return ret;
    }

   // if (regulator_count_voltages(fpsensor->vdd_regu) > 0) {
        ret = regulator_set_voltage(fpsensor->vdd_regu, 2800000,
                                    2800000);

        if (ret < 0 ) {
            printk("Regulator set_vtg failed vdd err = %d\n", ret);
            return ret;
        }
   // }
...
#endif
    if (fpsensor->vdd_regu == NULL) {
        printk(KERN_ERR, "ctl_dev->vdd_regu is NULL.\n");
        return (-ENODEV);
    }
    ret = regulator_enable(fpsensor->vdd_regu);

    if (ret) {
            printk(KERN_ERR, "Regulator vdd enable failed err = %d\n", ret);
            return ret;
    }
...
int fpsensor_spi_setup(fpsensor_data_t *fpsensor)
{
    ...
    FUNC_ENTRY();
	fpsensor->spi->max_speed_hz = FP_DEFAULT_SPI_SPEED;
...
#endif
{
	...
	fpsensor_hw_reset(1250);
	//fpsensor_gpio_output_dts(FPSENSOR_SPI_CS_PIN, 0);
	  fpsensor_dev->spi_freq_khz = 1000u;
          fpsensor_spi_setup(fpsensor_dev);
	hct_finger_set_spi_mode(1);
	udelay(50);
	r = tee_spi_transfer((void *) &fpsensor_spi_conf_mt65xx,sizeof(fpsensor_spi_conf_mt65xx), buf1, rbuf1, 2);
	if (r == 0) 
	{
		fpsensor_debug(INFO_LOG, "Fpsesnor read pass!!!!!!\n");
	}
	else
	{
		fpsensor_debug(INFO_LOG, "Fpsesnor read fail!!!!!!\n");
	}
	udelay(1000);
	r = tee_spi_transfer((void *) &fpsensor_spi_conf_mt65xx,sizeof(fpsensor_spi_conf_mt65xx), buf, rbuf, 5);
	if (r == 0) 
	{
		fpsensor_debug(INFO_LOG, "Fpsesnor read hardware id pass,hardware id is=0x%x,0x%x,0x%x !!!!!!\n",rbuf[0],rbuf[1],rbuf[2]);
	}
	else
	{
		fpsensor_debug(INFO_LOG, "Fpsesnor read hardware id fail!!!!!!\n");
	}
	//fpsensor_gpio_output_dts(FPSENSOR_SPI_CS_PIN, 1);
#if 1
	if(((rbuf[1]!=0x71)&&(rbuf[2]!=0x53))&&
		((rbuf[1]!=0x73)&&(rbuf[2]!=0x32)))
	{
		fpsensor_debug(ERR_LOG, "%s probe fail !!!!!!!!!!!!\n", __func__);
		fpsensor_remove(spi);
		return 0;
	}
#endif
...
#if 0
static struct spi_board_info fp_spi_board_devs[] __initdata = {
    [0] = {
        .modalias = "fpsensor-fp",
        .bus_num = 1,
        .chip_select = FINGERPRINT_FPFINGER_CS,
        .mode = SPI_MODE_0,
    },
};
#endif
...
#elif USE_SPI_BUS
    //fpret = spi_register_board_info(fp_spi_board_devs, ARRAY_SIZE(fp_spi_board_devs));
