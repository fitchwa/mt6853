...
int fpsensor_spidev_dts_init(fpsensor_data_t *fpsensor)
{
    ...
    int ret = 0;
    FUNC_ENTRY();
    node = of_find_compatible_node(NULL, NULL, "ontim,fingerprint");
    if (node) {
...
//------------------------------ree
	...
	fpsensor->vdd_regu = regulator_get(&fpsensor->spi->dev, "vfp");
   ...
   // if (regulator_count_voltages(fpsensor->vdd_regu) > 0) {
        ret = regulator_set_voltage(fpsensor->vdd_regu, 3000000,
                                    3000000);
   ...
   // }
    ...
    if (fpsensor->vdd_regu == NULL) {
        printk("ctl_dev->vdd_regu is NULL.\n");
        return (-ENODEV);
    }
