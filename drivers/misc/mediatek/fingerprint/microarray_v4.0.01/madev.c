...
static u8 *stxb = NULL;
static u8 *srxb = NULL;
static unsigned int MA_DRV_VERSION = 0x00004007;
...
//            wake_lock_timeout(&gProcessWakeLock, 5*HZ);
        ...
        case MA_IOC_GVER:
            mutex_lock(&ioctl_lock);
            retno = copy_to_user((unsigned int*)arg, &MA_DRV_VERSION, sizeof(unsigned int));
            mutex_unlock(&ioctl_lock);
            break;
		case MA_IOC_PWOF:
			mas_switch_power(0);
...
int init_connect(void){
    ...
    pr_info("hctfinger-%s: init_connect-1 ret=%d, srxb[0]=0x%x, srxb[1]=0x%x, srxb[2]=0x%x, srxb[3]=0x%x\n", __func__, ret, srxb[0], srxb[1], srxb[2], srxb[3]);
    msleep(5);
    for(i=0; i<4; i++){
        ...
        pr_info("hctfinger-%s: init_connect-2 ret=%d, srxb[0]=0x%x, srxb[1]=0x%x, srxb[2]=0x%x, srxb[3]=0x%x\n", __func__, ret, srxb[0], srxb[1], srxb[2], srxb[3]);
