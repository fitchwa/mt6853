...
#if defined(CONFIG_MTK_FLASHLIGHT_AW36428)
extern int aw36428_open_by_cam3(void);
extern int aw36428_close_by_cam3(void);
#endif
...
#define MainFakeFunc SensorFuncGC032AYUV_1
#else
...
atomic_t hct_cam_suspend;
#endif

#if __HCT_MAIN_FAKE_I2C_USE_SUB__ || __HCT_MAIN_FAKE_I2C_USE_SUB2__
...
MINT32 imgsensor_sensor_open(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if(psensor->inst.sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN)
	{
			if (ret == ERROR_NONE)
			{
				...
				#elif defined(GC032A_MIPI_YUV)
				strcpy(buf_yuv_name, SENSOR_DRVNAME_GC032A_MIPI_YUV);
		        #else
...
#if __HCT_MAIN_FAKE_I2C_USE_SUB__
{
				...
				if (psensor_mainfake != NULL) {
					...
					printk("zcy add [%s] psensor_mainfake is NULL.\n", __func__);
				}
}
#endif
...
#if __HCT_MAIN_FAKE_I2C_USE_SUB2__
{
				...
				if (psensor_mainfake != NULL) {
     			...
     			imgsensor_mutex_lock(psensor_inst);
#ifdef __HCT_FAKE_USE_MAIN3__
				gc0310_imgsensor_hw_power_sequence(&pimgsensor->hw, IMGSENSOR_SENSOR_IDX_MAIN3, IMGSENSOR_HW_POWER_STATUS_ON, sensor_power_sequence, buf_yuv_name);
#else
				...
				{
					printk("zcy add Main Fake %s SensorOpen Successfull", buf_yuv_name);
					hctfakeopen = 1;
#if __HCT_MAIN_FAKE_I2C_USE_SUB__ || __HCT_MAIN_FAKE_I2C_USE_SUB2__
...
#ifdef __HCT_FAKE_USE_MAIN3__
          		  gc0310_imgsensor_hw_power_sequence(&pimgsensor->hw, IMGSENSOR_SENSOR_IDX_MAIN3, IMGSENSOR_HW_POWER_STATUS_OFF, sensor_power_sequence, buf_yuv_name);
#else
...
#endif
				  printk("zcy add Main Fake %s SensorOpen failed!!!", buf_yuv_name);
       			}
				imgsensor_mutex_unlock(psensor_inst);
			}
	}
...
#if __HCT_SUB_FAKE_I2C_USE_MAIN__	//	hct-drv zhb 2018.1.29
{
				...
				if (psensor_subfake != NULL) {
					...
					printk("zcy add [%s] psensor_subfake is NULL.\n", __func__);
				}
}
#endif
        		imgsensor_mutex_lock(psensor_inst);
				...
				{
					printk("zcy add Sub Fake %s SensorOpen Successfull", buf_yuv_name);
					hctfakeopen = 1;
#if __HCT_SUB_FAKE_I2C_USE_MAIN__	//	hct-drv zhb 2018.1.29
					sub_fake_i2c_flag = 1;
#endif
				...
				else
		        {
				  ...
				  printk("zcy add Sub Fake %s SensorOpen failed!!!", buf_yuv_name);
        		}
	...
	if(psensor->inst.sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN)
		{
			...
			if (ret == ERROR_NONE)
			{
				...
				hctfakeopen = 0;
#if __HCT_MAIN_FAKE_I2C_USE_SUB__ || __HCT_MAIN_FAKE_I2C_USE_SUB2__	//	hct-drv zhb 2018.07.03
