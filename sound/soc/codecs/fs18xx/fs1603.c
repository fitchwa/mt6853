...
static int fs1603_store_otp(fsm_dev_t *fsm_dev, uint8_t valOTP)
{
	...
	do {
		...
		ret = fsm_reg_read(fsm_dev, REG(FSM_OTPRDATA), &otprdata);
...
int fs1603s_pre_f0_test(fsm_dev_t *fsm_dev)
{
	...
	// DC/DC mode: Boost; Vo=9.5V; Ilim: 3.8A
	ret |= fsm_reg_write(fsm_dev, REG(FSM_BSTCTRL), 0xBB1E);
	// Disable lnm
...
int fs1603s_f0_test(fsm_dev_t *fsm_dev)
{
	...
	if ((value & 0xC000) == 0x4000) { // bit[15:14] = 01
